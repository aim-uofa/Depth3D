...
if __name__=='__main__':
    ...
    data_root = '/mnt/nas/share/home/xugk/data/CO3D/CO3D'
    split_root = '/mnt/nas/share/home/xugk/data/CO3D/'

    files = []
    for scene_name in os.listdir(data_root):
        if scene_name == 'compressed':
            continue
            
        print(scene_name)
        scene_root = osp.join(data_root, scene_name)

        cam_metadata_file = osp.join(scene_root, 'frame_annotations.jgz')
        with gzip.GzipFile(cam_metadata_file, "rb") as f:
            cam_metadatas = load_dataclass(cast(IO, f), List[FrameAnnotation], binary=True)
        
        for i in range(len(cam_metadatas)):
            cam_metadata = cam_metadatas[i]
            K = co3d_annotation_to_opencv_pose(cam_metadata)
            fx = K[0][0]; fy = K[1][1]; cx = K[0][2]; cy = K[1][2]; 

        # for scene_id in os.listdir(scene_root):
        #     scene_id_root = osp.join(scene_root, scene_id)
        #     rgb_root = osp.join(scene_id_root, 'images')
        #     if not osp.exists(rgb_root):
        #         continue
            ...
            rgb_path = osp.join(data_root, cam_metadata.image.path)
            depth_path = osp.join(data_root, cam_metadata.depth.path)
            depth_mask_path = osp.join(data_root, cam_metadata.depth.mask_path)

            if not (osp.exists(rgb_path) and osp.exists(depth_path) and osp.exists(depth_mask_path)):
                continue
            
            # scale = cam_metadata.depth.scale_adjustment
            # if scale != 1:
            #     print('warning! scale != 1, continue...')
            #     continue
            ...
            meta_data['depth_mask_path'] = depth_mask_path.split(split_root)[-1]
            files.append(meta_data)

    files_dict = dict(files=files)

    with open('/mnt/nas/share/home/xugk/data/CO3D/train_annotations.json', 'w') as f:
