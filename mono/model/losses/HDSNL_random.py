...
class HDSNRandomLoss(nn.Module):
    ...
    """
    def __init__(self, loss_weight=1.0, random_num=32, data_type=['sfm', 'stereo', 'lidar'], sky_id=142, batch_limit=8, **kwargs):
    ...
    def get_random_masks_for_batch(self, image_size: list)-> torch.Tensor:
        ...
        mask_new = torch.zeros((self.random_num,  height, width), dtype=torch.bool, device="cuda") #[N, H, W]
    ...
    def forward(self, prediction, target, mask=None, sem_mask=None, **kwargs):
        ...
        if torch.isnan(loss).item() | torch.isinf(loss).item():
            # raise RuntimeError(f'HDSNL error, {loss}')
