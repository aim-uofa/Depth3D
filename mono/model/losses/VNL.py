...
class VNLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0, data_type=['sfm', 'stereo', 'lidar'], **kwargs):
        super(VNLoss, self).__init__()
        # self.fx = torch.tensor([focal_length], dtype=torch.float32).cuda()
        # self.fy = torch.tensor([focal_length], dtype=torch.float32).cuda()
    ...
    def get_loss(self, prediction, target, mask, intrinsic, select=True, **kwargs):
        ...
        if 'u_m_u0' not in self._buffers and 'v_m_v0' not in self._buffers:
