...
class RankingLoss(nn.Module):
    ...
    def forward(self, prediction, targets, masks=None, **kwargs):
        ...
        loss = 0.0 #torch.DoubleTensor([0.]).cuda()
        valid_samples = 0
        for i in range(n):
            # find A-B point pairs
            inputs_A, inputs_B, targets_A, targets_B, consistent_masks_A, consistent_masks_B = randomSampling(prediction[i,:], targets[i, :], masks[i, :], self.mask_value, self.point_pairs)
        ...
        if torch.isnan(loss).item() | torch.isinf(loss).item():
            raise RuntimeError(f'Ranking error, {loss}')
...
class EdgeguidedRankingLoss(nn.Module):
    ...
    def get_loss(self, prediction, target, mask=None, input=None, **kwargs):
        ...
        if n != 1:
            ...
            edges_depth = edges_depth.contiguous().view(1, -1)#.double()
            thetas_depth = thetas_depth.contiguous().view(1, -1)#.double()

        # initialization
        loss = 0.0 #torch.DoubleTensor([0.0]).cuda()
        valid_samples = 0

        for i in range(n):
            # Edge-Guided sampling
            inputs_A, inputs_B, targets_A, targets_B, masks_A, masks_B, sample_num = edgeGuidedSampling(prediction[i,:], targets[i, :], edges_img[i], thetas_img[i], masks[i, :], h, w)
            # Random Sampling
            random_sample_num = sample_num
            random_inputs_A, random_inputs_B, random_targets_A, random_targets_B, random_masks_A, random_masks_B = randomSampling(prediction[i,:], targets[i, :], masks[i, :], self.mask_value, random_sample_num)
