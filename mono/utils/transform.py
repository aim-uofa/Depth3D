...
class Compose(object):
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        for t in self.transforms:
            images, labels, intrinsics, cam_models, norms, other_labels, transform_paras = t(images, labels, intrinsics, cam_models, norms, other_labels, transform_paras)
        return images, labels, intrinsics, cam_models, norms, other_labels, transform_paras
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        if norms is not None:
            for i, norm in enumerate(norms):
                norms[i] = torch.from_numpy(norm.transpose(2, 0, 1)).float()
...
class ResizeCanonical(object):
    ...
    def random_on_crop_transform(self, image, label, intrinsic, cam_model, to_random_ratio):
        ...
        # random resize based on the last crop_size
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        for i in range(len(images)):
            ...
            img, label, intrinsic, cam_model = self.random_on_crop_transform(
        ...
        if norms != None:
            ...
            for i, norm in enumerate(norms):
                norms[i] = cv2.resize(norm, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_NEAREST)
        ...
        # if ref_images is not None:
        #     for i, ref_i in enumerate(ref_images):
        #         ref_images[i] =  cv2.resize(ref_i, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_LINEAR)
...
class ResizeKeepRatio(object):
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        if norms is not None:
            for i, norm in enumerate(norms):
                norm = cv2.resize(norm, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_NEAREST)
                # pad
                norms[i] =  cv2.copyMakeBorder(
                    norm, 
                    pad_h_half, 
                    pad_h - pad_h_half, 
                    pad_w_half, 
...
class ResizeKeepRatio_wo_Padding(object):
    """
    Resize and pad to a given size. Hold the aspect ratio.
    This resizing assumes that the camera model remains unchanged.
    Args:
...
class Resize(object):
    ...
    def main_data_transform(self, image, label, intrinsic, cam_model, resize_ratio_h, resize_ratio_w, padding, to_scale_ratio):
        """
        Resize data first and then do the padding.
        'label' will be scaled.
        """
        h, w, _ = image.shape
        reshape_h = self.resize_h
        reshape_w = self.resize_w
        ...
        # Resize, adjust principle point
        if intrinsic is not None:
            intrinsic[0] = intrinsic[0] * resize_ratio_w / to_scale_ratio
            intrinsic[1] = intrinsic[1] * resize_ratio_h / to_scale_ratio
            intrinsic[2] = intrinsic[2] * resize_ratio_w
            intrinsic[3] = intrinsic[3] * resize_ratio_h
    ...
    def get_label_scale_factor(self, image, intrinsic, resize_ratio_h, resize_ratio_w):
        ori_h, ori_w, _ = image.shape
        # crop_h, crop_w = self.crop_size
        ori_focal = intrinsic[0]

        to_canonical_ratio = self.canonical_focal / ori_focal
        # NOTE: (resize_ratio_h + resize_ratio_w) / 2 is not correct here!!!! but how to solve it?
        to_scale_ratio = (resize_ratio_h + resize_ratio_w) / 2 / to_canonical_ratio
        return to_scale_ratio
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        pad_info = [pad_h, pad_w, pad_h_half, pad_w_half]
        to_scale_ratio = self.get_label_scale_factor(images[0], intrinsics[0], resize_ratio_h, resize_ratio_w)
        ...
        for i in range(len(images)):
            ...
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model = self.main_data_transform(
                img, label, intrinsic, cam_model, resize_ratio_h, resize_ratio_w, pad_info, to_scale_ratio)
            images[i] = img
            if label is not None:
                labels[i] = label
            if intrinsic is not None:
...
class Resize_wo_depth_scale(object):
    """
    Resize and pad to a given size. Hold the aspect ratio.
    This resizing assumes that the camera model remains unchanged.
    Args:
    ...
    def main_data_transform(self, image, label, intrinsic, cam_model, resize_ratio_h, resize_ratio_w, padding):
        """
        Resize data first and then do the padding.
        'label' will be scaled.
        """
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        for i in range(len(images)):
            ...
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model = self.main_data_transform(
                img, label, intrinsic, cam_model, resize_ratio_h, resize_ratio_w, pad_info)
            images[i] = img
            if label is not None:
                labels[i] = label
            if intrinsic is not None:
...
class RandomCrop(object):
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        if norms is not None:           
            for i, norm  in enumerate(norms):
                # padding if current size is not satisfied
                norm = cv2.copyMakeBorder(norm, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=0)
                norms[i] = norm[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        if other_labels is not None:           
            for i, other_lab  in enumerate(other_labels):
                # padding if current size is not satisfied
...
class RandomCropRatio(object):
    ...
    Args:
        ...
        int instead of sequence like (h, w), a square crop (size, size) is made.
    """
    def __init__(self, crop_ratio, crop_type='center', padding=None, ignore_label=-1, **kwargs):
        if isinstance(crop_ratio, float):
            self.crop_ratio_low = crop_ratio
            self.crop_ratio_high = crop_ratio
        elif isinstance(crop_ratio, collections.Iterable) and len(crop_ratio) == 2 \
                and isinstance(crop_ratio[0], float) and isinstance(crop_ratio[1], float) \
                and crop_ratio[0] > 0 and crop_ratio[1] > 0:
            self.crop_ratio_low = crop_ratio[0]
            self.crop_ratio_high = crop_ratio[1]
        else:
            raise (RuntimeError("crop size error.\n"))
        if crop_type == 'center' or crop_type == 'rand' or crop_type=='rand_in_field':
            self.crop_type = crop_type
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        target_img = images[0]
        target_h, target_w, _ = target_img.shape
        self.crop_h = int(target_h * random.uniform(self.crop_ratio_low, self.crop_ratio_high))
        self.crop_w = int(target_w * random.uniform(self.crop_ratio_low, self.crop_ratio_high))
...
class RandomResize(object):
    ...
    Args:
        images: list of RGB images.
        labels: list of depth/disparity labels.
        other labels: other labels, such as instance segmentations, semantic segmentaions...
    """
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        assert len(images[0].shape) == 3 and len(labels[0].shape) == 2
        labels[0] = labels[0].astype(np.float32)
        assert labels[0].dtype == np.float32
        # target_focal = (intrinsics[0][0] + intrinsics[0][1]) / 2.0
        # target_to_canonical_ratio = self.canonical_focal / target_focal
        # target_img_shape = images[0].shape
...
class RandomEdgeMask(object):
    '''
    Ramdom mask the input and labels.
    Args:
        images: list of RGB images.
        labels: list of depth/disparity labels.
        other labels: other labels, such as instance segmentation, semantic segmentations...
    '''
    def __init__(self, mask_maxsize=32, prob=0.5, rgb_invalid=[0,0,0], label_invalid=-1, **kwargs) -> None:
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        if norms != None: 
            for i, norm in enumerate(norms):
                norms[i] = self.mask_edge(norm, mask_edgesize, 0)
...
class AdjustSize(object):
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        if norms is not None:
            ...
            for i, norm  in enumerate(norms):
                norms[i] = cv2.copyMakeBorder(norm, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=0)
        ...
        if norms is not None:
            for i, norm in enumerate(norms):
                if rotate:
                    ...
                    norm[:, :, 0] = -norm[:, :, 0] # NOTE: check the direction of normal coordinates axis, this is ued in https://github.com/baegwangbin/surface_normal_uncertainty
                norms[i] = norm
...
class LiDarResizeCanonical(object):
    ...
    def random_on_crop_transform(self, image, label, intrinsic, cam_model, to_random_ratio):
        ...
        if label is not None:
            # number of other labels may be less than that of image
            label = resize_depth_preserve(label, (reshape_w, reshape_h))
            # scale the label and camera intrinsics
            label = label / to_scale_ratio
    ...
    def __call__(self, images, labels, intrinsics, cam_models=None, norms=None, other_labels=None, transform_paras=None):
        ...
        for i in range(len(images)):
            ...
            intrinsic = intrinsics[i] if i < len(intrinsics) else None
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model, to_scale_ratio = self.random_on_crop_transform(
                img, label, intrinsic, cam_model, to_random_ratio)
...
if __name__ == '__main__':
    img = cv2.imread('NuScenes/data_test/samples/CAM_FRONT/n008-2018-08-01-15-34-25-0400__CAM_FRONT__1533152214512404.jpg', -1)
    H, W, _ = img.shape
    label = img[:, :, 0]
...
#           # this NEAREST resize may lost most of information
#           #label = cv2.resize(label, size, interpolation=cv2.INTER_NEAREST)
#          #label = resize_depth_preserve(label, size)
#         # print(image.shape, label.shape, old_image_shape, old_label_shape)
#         # exit()
#         # print(image.shape, label.shape)
#         return image, label

# class ResizeWhole(object):
#     # Resize the input to the given size, 'size' is a 2-element tuple or list in the order of (h, w).
...
#             self.scale = scale
#         else:
#             raise (RuntimeError("depthtransform.RandScale() scale param error.\n"))
...
#             self.aspect_ratio = aspect_ratio
#         else:
#             raise (RuntimeError("depthtransform.RandScale() aspect_ratio param error.\n"))
...
#     def __call__(self, image, label, intrinsic, cam_model=None, ref_images=None, transform_paras=None):
#         assert len(image.shape) == 3 and len(label.shape) == 2
...
#         if ref_images is not None:
#             for i, ref_i in enumerate(ref_images):
#                 assert len(ref_i.shape) == 3
#                 ref_images[i] =  cv2.resize(ref_i, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_LINEAR)
            

#         return image, label, intrinsic, cam_model, ref_images, transform_paras