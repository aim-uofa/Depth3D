"""
https://github.com/guangkaixu/BoostingDepth
"""
...
def sparse_depth_lwlr_batch(pred_mono, guided_depth, sample_mode='grid', sample_num=100, down_sample_scale=32, lambda_limit_value=1, dataset_name=None, k_para=None, device=torch.device('cpu')):
    '''
    pred_mono:          monocular depth estimation, with shape: [B, H, W]
    guided_depth:       sparse guided points, from gt or low quality sensors, with shape: [B, H, W] and 0 for invalid area.
    ...
    # mask = (target > 1e-8)
    # NOTE: mask = (target != 0) only suitable for reconstruction optimization
    mask = (target != 0)
