...
if __name__ == '__main__':
    ...
    import json
    print('Input dataset_num here:')
    dataset_num = input()
    anno_info = data_info[dataset_num]

    try:
        train_anno_path = osp.join(anno_info['database_root'], anno_info['train_annotations_path'])
        with open(train_anno_path, 'r') as f1:
            train_annos = json.load(f1)
        try:
    ...
    try:
        test_anno_path = osp.join(anno_info['database_root'], anno_info['test_annotations_path'])
        with open(test_anno_path, 'r') as f2:
            test_annos = json.load(f2)
        try:
